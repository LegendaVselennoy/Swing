...
public class DialogDemo extends Application {
    ...
    public void start(Stage stage) throws Exception {
        ...
        information.setOnAction(event -> {
            Alert alert=new Alert(Alert.AlertType.INFORMATION,"Everything is fine.");
            alert.showAndWait();
        });

        Button warning=new Button("Warning");
        warning.setOnAction(event -> {
            Alert alert=new Alert(Alert.AlertType.WARNING,"We have a problem.");
            alert.showAndWait();
        });
        ...
        multipleButtons.setOnAction(event -> {
            Alert alert=new Alert(Alert.AlertType.NONE,"Now what?",
                    ButtonType.NEXT,
                    ButtonType.PREVIOUS,
                    ButtonType.FINISH);
...
//            alert.setGraphic(new ImageView("bomb.png"));
        ...
        textInput.setOnAction(event -> {
            ...
            dialog.showAndWait().ifPresentOrElse(
                    result->textArea.appendText("Name: "+result+"/n"),
                    ()->textArea.appendText("Canceled\n"));
        });
        ...
        fileChooser.setOnAction(event -> {
            ...
            dialog.getExtensionFilters().addAll(new FileChooser.ExtensionFilter("GIF images","*.gif"),
        ...
        VBox buttons=new VBox(0.8*rem,information,warning,error,confirmation,multipleButtons,expandableContent,
                textInput,choiceDialog,fileChooser,directoryChooser);
