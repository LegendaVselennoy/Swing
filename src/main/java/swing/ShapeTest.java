...
/**
 * В этой программе демонстрируется построение различных двумерных фигур
 */
...
/**
 * Этот фрейм содержит комбинированный список для выбора фигур, а также компонент для их рисования
 */
...
/**
 * Этот компонент рисует фигуру и даёт пользователю возможность перемещать определяющие её очки
 */
class ShapeComponent extends JComponent{
    ...
    /**
     * Устанавливает построитель фигур и инициализирует его производным рядом точек
     * @param aShapeMaker Построитель фигур, определяющий фигуру по ряду исходных точек
     */
...
/**
 * Построитель фигур по ряду исходных точек. Конкретные подклассы должны возвращать форму из метода makeShape()
 */
abstract class ShapeMaker{
     ...
     * @param pointCount Количество исходных точек, требующихся для определения данной фигуры
     */
    ...
    /**
     * Получает количество исходных точек, требующихся для определения данной фигуры
     * @return Количество исходных точек
     */
    ...
    /**
     * Строит фигуру по заданному ряду исходных точек
     * @param p Исходные точки, определяющие фигуру
     * @return Фигура, определяемая исходными точками
     */
...
/**
 * Строит прямоугольник со скругленными углами, соединяющий две заданные угловые точки
 */
...
/**
 * Строит эллипс, вписанный в ограничивающий прямоугольник с двумя заданными угловыми точками
 */
...
/**
 * Строит дугу, вписанную в ограничивающий прямоугольник с двумя заданными угловыми точками.
 * Начальный и конечный углы дуги задают линии, проведённые из центра ограничивающего прямоугольника
 * в две заданные точки. Для демонстрации правильности расчёта углов возвращаемая фигура состоит из дуги,
 * ограничивающего прямоугольника и линий, образующих эти углы
 */
class ArcMaker extends ShapeMaker{
    ...
    public Shape makeShape(Point2D[]p){
        ...
        var s=new Arc2D.Double(0,0,0,0,skewedStartAngle,skewedAngleDifference,Arc2D.OPEN);
        s.setFrameFromDiagonal(p[0],p[1]);
...
/**
 * Строит кривую второго порядка, определяемую двумя конечными точками и одной управляющей точкой
 */
class QuadCurveMaker extends ShapeMaker{
    ...
    public Shape makeShape(Point2D[]p){
        return new QuadCurve2D.Double(
                p[0].getX(),p[0].getY(),p[1].getX(),
                p[1].getY(),p[2].getX(),p[2].getY());
    }
}

/**
 * Строит кривую третьего порядка, определяемую двумя конечными точками и двумя управляющими точками
 */
